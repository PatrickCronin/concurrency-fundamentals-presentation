...
# Constants
Readonly my $HIT_COUNT_LOCKFILE => '/Users/pcronin/Projects by Client/Cronin Technology Consulting, LLC/Projects by Client/Ruby Brigade/Concurrency/Website Hit Counter/hit_count.lock';
...
Readonly my $SAFE_LOG_FILE => '/Users/pcronin/Projects by Client/Cronin Technology Consulting, LLC/Projects by Client/Ruby Brigade/Concurrency/Website Hit Counter/safe.log';
...
# Increment the hit count in the hit count file by 1, safely!
sub increment_hit_count {
    open HIT_COUNT_LOCK, '>', $HIT_COUNT_LOCKFILE;
    flock HIT_COUNT_LOCK, LOCK_EX;

    my $current_hit_count = read_hit_count();
    $current_hit_count++;
    write_hit_count($current_hit_count);
    
    flock HIT_COUNT_LOCK, LOCK_UN;
    close HIT_COUNT_LOCK;
}
...
sub read_hit_count {
    ...
    safe_log("Read $current_hit_count from hit count file.");
...
sub write_hit_count {
    ...
    safe_log("Wrote $new_hit_count to hit count file.");
}

# Truncate the log file
sub clear_log {
    open LOG, '>', $SAFE_LOG_FILE;
    close LOG;
}
